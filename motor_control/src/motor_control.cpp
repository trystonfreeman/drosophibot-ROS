...
private:
    ...
    void timer_callback()
    {
        ...
        for (uint8_t id = 0; id < 22; id++){
			...
			packetHandler->read4ByteTxRx(portHandler, id, present_velocity_address, &velocity);
			message.pos[id] = position;
			message.vel[id] = velocity;
...
int main(int argc, char * argv[])
{
	...
	portHandler->setBaudRate(1000000);
 	for (int id = 0; id < 22; id++){
		packetHandler->write1ByteTxRx(portHandler, id, torque_on_address, data);
	}
